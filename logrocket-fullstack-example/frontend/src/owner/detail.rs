...
impl Component for Detail {
    ...
    fn update(
        ...
        match msg {
            Msg::MakePetsReq(id) => {
                let req = Request::get(
                    &format!("http://localhost:8000/owner/{}/pet", id)
                )
                    .body(Nothing)
            ...
            Msg::MakeOwnerReq(id) => {
                let req = Request::get(
                    &format!("http://localhost:8000/owner/{}", id)
                )
                    .body(Nothing)
                    .expect("Can make request to backend");
            ...
            Msg::MakeDeletePetReq(owner_id, pet_id) => {
                let req = Request::delete(
                    &format!("http://localhost:8000/owner/{}/pet/{}", owner_id, pet_id)
                )
                    .body(Nothing)
                    .expect("Can make request to backend");
        ...
        html! {
            <div class=classes!("list-item", "pet")>
                <div>
                    ...
                    { " (" }
                    <button onclick=self.link.callback(move |_| Msg::MakeDeletePetReq(owner_id, id))>
                        { "Delete Pet" }
                    </button>
                    { ")" }
                </div>
